...
transitive_rule = IF( AND( '(?a) beats (?b)',
                           '(?b) beats (?c)' ),
                      THEN( '(?a) beats (?c)' ) )
...
# you need to.
rule_same = IF( OR( 'male (?x)',
                 'female (?x)' ),
             THEN( 'same-identity (?x) (?x)') )
rule_sibling = IF( AND( 'parent (?a) (?b)',
                  'parent (?a) (?c)',
                  NOT( 'same-identity (?b) (?c)' ) ),
             THEN( 'sibling (?b) (?c)' ) )
rule_brother = IF( AND( 'sibling (?x) (?y)',
                   'male (?x)' ),
              THEN( 'brother (?x) (?y)' ) )
rule_sister = IF( AND( 'sibling (?x) (?y)',
                  'female (?x)' ),
             THEN( 'sister (?x) (?y)' ) )
rule_cousin = IF( AND( 'sibling (?a) (?b)',
                  'parent (?a) (?c)',
                  'parent (?b) (?d)' ),
             THEN( 'cousin (?c) (?d)' ) )
rule_father = IF( AND( 'parent (?x) (?y)',
                       'male (?x)' ),
                  THEN( 'father (?x) (?y)' ) )
rule_mother = IF( AND( 'parent (?x) (?y)',
                       'female (?x)' ),
                  THEN( 'mother (?x) (?y)' ) )
rule_son = IF( AND( 'parent (?x) (?y)',
                       'male (?y)' ),
                  THEN( 'son (?y) (?x)' ) )
rule_daughter = IF( AND( 'parent (?x) (?y)',
                       'female (?y)' ),
                  THEN( 'daughter (?y) (?x)' ) )
rule_grand = IF( AND( 'parent (?a) (?b)',
                      'parent (?b) (?c)' ),
                 THEN( AND( 'grandparent (?a) (?c)',
                            'grandchild (?c) (?a)' ) ) )

# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [rule_same,
                rule_sibling,
                rule_brother,
                rule_sister,
                rule_cousin,
